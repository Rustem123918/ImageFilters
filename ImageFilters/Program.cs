...
namespace Recognizer
{
    internal static class Program
    {
        ...
        private static void Main()
        {
            ...
            var clear = MedianFilter.DoMedianFilter(grayscale);
            panel.Controls.Add(CreateBox(ConvertToBitmap(clear)), 2, 0);
            var sobell = SobelFilter.DoSobelFilter(clear, new double[,] { { -1, -2, -1 }, { 0, 0, 0 }, { 1, 2, 1 } });
            panel.Controls.Add(CreateBox(ConvertToBitmap(sobell)), 0, 1);
            var trashhold = ThresholdFilter.DoThresholdFilter(sobell, 0.1);
